...
RegisterNetEvent("LSC:menumechanicreturn")
AddEventHandler("LSC:menumechanicreturn", function(source)
	ismechanic = 1
end)

RegisterNetEvent("LSC:menumechanicreturnfalse")
AddEventHandler("LSC:menumechanicreturnfalse", function(source)
	ismechanic = 0
end)

function fixtheprice(price)
	if ismechanic == 1 then
		return price * 0.5
	end
...
function Menu:addPurchase(name,price,info,args)
	...
	button.price = string.format("%.0f",(fixtheprice(price)))
