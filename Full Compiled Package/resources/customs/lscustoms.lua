...
local currentpolicegarage = 0
local ismechanic = 0

local garages = { 
	[1] = { locked = false, drivein = {x = -337.32, y = -136.68, z = 38.82, heading = 22.1}},
	[2] = { locked = false, drivein = {x = 733.69,y = -1088.74, z = 21.733, heading = 22.1}},
	[3] = { locked = false, drivein = {x = -1155.077,y = -2006.61, z = 12.465, heading = 22.1}},
	[4] = { locked = false, drivein = {x = 1174.701,y = 2643.764, z = 37.048, heading = 22.1}},
	[5] = { locked = false, drivein = {x = 112.738,y = 6624.644, z = 31.072, heading = 22.1}},
	[6] = { locked = false, drivein = {x = -211.72, y = -1325.3, z = 30.7, heading = 22.1}},
	[7] = { locked = false, drivein = {x = 1768.54, y = 3329.67, z = 41.44, heading = 22.1}},
	[8] = { locked = false, drivein = {x = 1143.21, y = -778.24, z = 57.44, heading = 22.1}},
	[9] = { locked = false, drivein = {x = 541.37, y = -183.12, z = 54.29, heading = 22.1}},
	[10] = { locked = false, drivein = {x = 2501.51, y = 4081.12, z = 37.29, heading = 22.1}},
	[11] = { locked = false, drivein = {x = 1640.68, y = 4840.12, z = 41.29, heading = 22.1}},
	[12] = { locked = false, drivein = {x = 965.28, y = -108.93, z = 74.26, heading = 22.1}},
	[13] = { locked = false, drivein = {x = -6.54, y = -2764.67, z = 0.00, heading = 22.1}},
}

local copgarages = { 
  [1] = { copgarageblip = {x = 462.13, y = -1017.44, z = 28.08, heading = 102.213}},
  [2] = { copgarageblip = {x = 1868.42, y = 3699.08, z = 33.3, heading = 102.213}},
  [3] = { copgarageblip = {x = 823.58, y = -1371.39, z = 25.82, heading = 102.213}},
  [4] = { copgarageblip = {x = 377.75, y = -1613.53, z = 28.98, heading = 102.213}},
  [5] = { copgarageblip = {x = 538.97, y = -41.08, z = 70.47, heading = 102.213}},
  [6] = { copgarageblip = {x = -1070.49, y = -854.63, z = 4.55, heading = 102.213}},
}
...
local repairMult = 1
RegisterNetEvent('sendCustomPricing')
AddEventHandler('sendCustomPricing', function(veh)
	vehPrices = veh
end)

function setRepairPrice(veh)
	--Citizen.CreateThread(function()
		--lsc.menu["repair"].buttons = {}
		vehPrices = nil
		TriggerServerEvent('getVehPricing')
		while vehPrices == nil do
			Citizen.Wait(0)
		end
		local vehPrice = repairMult
		local vehBody = 1000 - GetVehicleBodyHealth(veh)
		local vehEngine = 1000 - GetVehicleEngineHealth(veh)
		for i = 1, #vehPrices do
			if GetHashKey(vehPrices[i].model) == GetEntityModel(veh) then
				vehPrice = vehPrices[i].costs / 20000
			end
		end
		local damageTotal = vehBody + vehEngine
		local vehTotal = math.ceil(damageTotal * vehPrice)
		--table.insert(lsc.menu["repair"].buttons, {name = "Repair vehicle", description = "Full body repair and engine service.", costs = vehTotal, centre = 0, font = 0, scale = 0.4})
		TriggerServerEvent('setPlayerRepairPrice', vehTotal)
		--print(lsc.menu.repair.buttons.costs)
		return vehTotal
	--end)
end
...
local function DriveInGarage()
	...
	if DoesEntityExist(veh) then
		...
		elseif currentgarage == 7 then
			LSCMenu:setTitle("Highwayman's Auto Repair")
			LSCMenu.title_sprite = "shopui_title_sm_hangar"
		else
			LSCMenu:setTitle("Los Santos Customs")
			LSCMenu.title_sprite = "shopui_title_carmod"
		end
		if currentpolicegarage == 1 then
			LSCMenu:setTitle("Jerry's Garage")
			LSCMenu.title_sprite = "shopui_title_sm_hangar"
		end
		...
		local repairCost = setRepairPrice(veh)
		...
		AddMod(16,LSCMenu.categories.Lights,"XENON COLOUR","Xenon Colour","Customize your xenon's colour.          (ONLY FOR THIS SESSION)",nil, false)
		...
		Citizen.CreateThread(function()
			...
			if GetVehicleBodyHealth(veh) < 1000 or GetVehicleEngineHealth(veh) < 1000 then
				LSCMenu:Open("main")
			else
...
local function checkifmechanic()
	TriggerServerEvent("LSC:mechaniccheck", src)
end

RegisterNetEvent("LSC:mechanicreturn")
AddEventHandler("LSC:mechanicreturn", function(source)
	ismechanic = 1
	print ("i am a mechanic")
end)

RegisterNetEvent("LSC:mechanicreturnfalse")
AddEventHandler("LSC:mechanicreturnfalse", function(source)
	ismechanic = 0
	print ("im not a mechanic")
end)
	...
	while true do
		...
		if inside == false then
			...
			if IsPedSittingInAnyVehicle(ped) then
				...
				if DoesEntityExist(veh) and GetPedInVehicleSeat(veh, -1) == ped then -- and (IsThisModelACar(GetEntityModel(veh)) or IsThisModelABike(GetEntityModel(veh)))  then
					...
					for i,pos in ipairs(garages) do
						...
						if LSC_Config.oldenter then			
							...
							if GetDistanceBetweenCoords(outside.x,outside.y,outside.z,GetEntityCoords(ped)) <= f(8) then
								if IsControlJustPressed(1,38) then
									...
									ShowNotification('Press ~INPUT_PICKUP~ to enter customs')
								end
							end
						else
	...
	while true do
		...
		if inside == false then
			...
			if IsPedSittingInAnyVehicle(ped) then
				...
				if DoesEntityExist(veh) and GetPedInVehicleSeat(veh, -1) == ped then -- and (IsThisModelACar(GetEntityModel(veh)) or IsThisModelABike(GetEntityModel(veh)))  then
					--So lets go through every garage
					for i,pos in ipairs(copgarages) do
						--Lets take the outside coords of garage
						outside = pos.copgarageblip
						--Old enter:If vehicle is close enough, then text will be displayed - Press ENTER to enter garage
						if LSC_Config.oldenter then			
							--So, if vehicle is close enough then we can continue
							if GetDistanceBetweenCoords(outside.x,outside.y,outside.z,GetEntityCoords(ped)) <= f(8) then
								--Lets check if our vehicle is not in the model black list, and if it is not then we can go further
								if tableContains(LSC_Config.ModelBlacklist,GetDisplayNameFromVehicleModel(GetEntityModel(veh)):lower()) then
									...
									drawTxt("~r~This vehicle can't be modified here",4,1,0.5,0.8,0.8,0,0,0,255)
								end
							end
						end
					end
...
function LSCMenu:onButtonSelected(name, button)
	if  tableContains(LSC_Config.ModelBlacklist,GetDisplayNameFromVehicleModel(GetEntityModel(GetVehiclePedIsUsing(GetPlayerPed(-1)))):lower()) then
		TriggerServerEvent("LSC:buttonSelectedPolice", name, button)
	else
...
AddEventHandler("LSC:buttonSelected", function(name, button, canpurchase)
	...
	elseif mname == "xenon colour" then
		if button.name == "Stock" or button.purchased or CanPurchase(price, canpurchase)then
			myveh.mods[button.modtype].mod = button.mod
...
function CheckPurchases(m)
	...
	elseif name == "armor" or name == "suspension" or name== "xenon colour" or name == "transmission" or name == "brakes" or name == "engine tunes" or name == "roof" or name == "fenders" or name == "hood" or name == "grille" or name == "roll cage" or name == "exhausts" or name == "skirts" or name == "rear bumpers" or name == "front bumpers" or name == "spoiler" then
		for i,b in pairs(m.buttons) do
			if b.mod == -1  then
