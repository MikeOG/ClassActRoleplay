local phoneNumbers = {}
local phoneNumbersToIdentifiers = {}
local sourceToIden = {}
local idenToSource = {}

math.randomseed(os.time())

AddEventHandler("Phones.AddCharacterNumber", function(src, num, identifier)
	phoneNumbers[identifier] = num
	phoneNumbersToIdentifiers[num] = identifier
	sourceToIden[src] = identifier
	idenToSource[identifier] = src

	TriggerClientEvent("gcPhone:myPhoneNumber", src, num)
	TriggerClientEvent("gcPhone:contactList", src, getContacts(identifier))
	TriggerClientEvent("gcPhone:allMessage", src, getMessages(identifier))
...
function getSourceFromIdentifier(identifier, cb)
	cb(idenToSource[identifier])
end
function getNumberPhone(identifier)
	return phoneNumbers[identifier]
end
function getIdentifierByPhoneNumber(phone_number)
	return phoneNumbersToIdentifiers[phone_number]
end

function getPlayerID(source)
	return sourceToIden[source]
end
...
function addContact(source, identifier, number, display)
	...
	if not string.find("555-", number) then
		number = "555-"..number
	end
...
function getMessages(identifier)
	...
	local result = MySQL.Sync.fetchAll("SELECT phone_messages.* FROM phone_messages LEFT JOIN character_data ON character_data.CharID = @identifier WHERE phone_messages.receiver = @num AND phone_messages.isDeleted = false", {
		['@identifier'] = identifier,
		['@num'] = num
	})
	return result
	--return MySQLQuerytime("SELECT phone_messages.* FROM phone_messages LEFT JOIN users ON users.identifier = @identifier WHERE phone_messages.receiver = users.phone_number", {['@identifier'] = identifier})
end
...
function _internalAddMessage(transmitter, receiver, message, owner)
	...
	local Query2 = 'SELECT * from phone_messages WHERE `transmitter` = @trans ORDER BY id DESC'
	local Parameters = {
	...
	return MySQL.Sync.fetchAll(Query2, {['@trans'] = transmitter})[1]
end
...
function deleteMessage(msgId)
	...
	MySQL.Sync.execute("UPATE phone_messages SET isDeleted = true WHERE `id` = @id", {
		['@id'] = msgId
	})
end
...
function deleteAllMessageFromPhoneNumber(source, identifier, phone_number)
	...
	MySQL.Sync.execute("UPDATE phone_messages SEt isDeleted = true WHERE `receiver` = @mePhoneNumber and `transmitter` = @phone_number", {['@mePhoneNumber'] = mePhoneNumber,['@phone_number'] = phone_number})
end

function deleteAllMessage(identifier)
	local mePhoneNumber = getNumberPhone(identifier)
	--MySQL.Sync.execute("DELETE FROM phone_messages WHERE `receiver` = @mePhoneNumber", {
	MySQL.Sync.execute("UPDATE phone_messages SET isDeleted = true WHERE `receiver` = @mePhoneNumber", {
...
AddEventHandler('onMySQLReady', function ()
	MySQL.Async.fetchAll("DELETE FROM phone_messages WHERE (DATEDIFF(CURRENT_DATE,time) > 30)")
end)
